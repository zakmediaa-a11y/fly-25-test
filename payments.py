...
DODO_BASE_URL = os.getenv("DODO_BASE_URL", "https://test.dodopayments.com")
...
try:
    ...
    st.error(f"❌ Failed to initialize Supabase client in payments: {e}")
...
def create_checkout_session(user_id: str, user_email: str, plan_type: str) -> Tuple[bool, str, str]:
    ...
    try:
        ...
        # Check if DODO_API_KEY is available
        if not DODO_API_KEY:
            return False, "Payment service not configured", None
...
# ... rest of payments.py remains the same, just update any other type hints if needed
...
def show_pricing_page():
    ...
    # Check if coming back from successful payment (compatible with older Streamlit)
    try:
        # Try new Streamlit query_params first
        if hasattr(st, 'query_params'):
            query_params = st.query_params
            if 'payment_success' in query_params:
                st.success("🎉 Payment successful! Your subscription is now active!")
                plan_type = query_params.get('plan', 'monthly')
        ...
        # Fallback for older Streamlit versions
            ...
            if 'payment_success' in query_params:
                st.success("🎉 Payment successful! Your subscription is now active!")
                plan_type = query_params.get('plan', ['monthly'])[0]
                user_id = st.session_state.user['id']
                activate_subscription(user_id, plan_type, is_trial=True)
                st.rerun()
    except Exception as e:
        # Silently continue if query params can't be read
        pass
    
    cols = st.columns(4)
    
    current_sub = st.session_state.get('subscription', {})
    ...
    # FAQ and comparison table remain the same...
    # ... (keep the existing FAQ and comparison table code)
