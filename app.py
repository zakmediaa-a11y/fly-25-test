...
def calculate_confidence(self, result: 'EmailVerificationResult') -> int:
    ...
    if result.smtp_verified is True:
        ...
        return 0  # Definitely invalid if SMTP says no
    # If SMTP is None (inconclusive), don't penalize
    
    if result.is_catch_all:
        score -= 10
    if result.is_disposable:
        score = min(score, 50)  # Cap disposable emails
    if result.is_role_based:
        score -= 5
    
    # Boost score for free providers (they usually have good validation)
    if result.is_free_provider and result.mx_records_exist:
        score += 10
...
def verify(self, email: str, check_smtp: bool = True, check_catch_all: bool = True) -> EmailVerificationResult:
    ...
    if smtp_verified is True:
        ...
        status = 'RISKY'
        deliverable = smtp_verified is not False  # Assume deliverable unless proven otherwise
    elif is_catch_all:
        status = 'RISKY'
        deliverable = True
    else:
        # If SMTP verification is inconclusive but domain and MX exist,
        # give it a higher chance of being valid
        status = 'LIKELY_VALID'
        deliverable = True
...
def show_find_tab(verifier, user, delay):
    ...
    if find_btn:
        ...
        for idx, line in enumerate(lines):
            ...
            # First pass: Look for SMTP-verified emails
            for p in patterns:
                result = verifier.verify(p['email'])
                
                # If we find an SMTP-verified email, use it immediately
                if result.smtp_verified is True:
                    verified_email = p['email']
                    best_result = result
                    break
                
                # Otherwise track the best candidate (highest confidence)
            ...
            # Use verified email if found, otherwise use best candidate
            final_email = verified_email if verified_email else best_email
            ...
            if final_result:
                results.append({
                    'Name': f"{first} {last}",
                    'Domain': domain,
                    'Found_Email': final_email,
                    'Status': final_result.status,
                    'SMTP_Verified': '✅' if final_result.smtp_verified else '❌',
                    'Confidence': f"{final_result.confidence_score}%",
                    'Method': 'SMTP Verified' if verified_email else 'Best Guess'
                })
        
        df = pd.DataFrame(results)
        
        # Count actual verified emails
        verified_count = sum(1 for r in results if r.get('SMTP_Verified') == '✅')
        total_attempted = len(lines)
        
        st.success(f"🎯 Complete! Found {verified_count}/{total_attempted} verified emails")
...
def show_bulk_find_tab(verifier, user, delay):
    """Bulk find emails from CSV with names and domains"""
    st.markdown("### 📁 Bulk Find Emails from CSV")
    st.caption("Upload a CSV with FirstName, LastName, Domain columns to find emails in bulk")
    
    uploaded_file = st.file_uploader("Upload CSV", type=['csv'], key="bulk_find")
    ...
    if uploaded_file:
        try:
            ...
            if missing_cols:
                ...
                st.info("Your CSV should have columns: FirstName, LastName, Domain")
                return
            
            if st.button("🔍 Find Emails in Bulk", type="primary"):
                results = []
                total_rows = len(df)
                ...
                for idx, row in df.iterrows():
                    status_text.text(f"Processing {idx + 1}/{total_rows}: {row['FirstName']} {row['LastName']} @ {row['Domain']}")
                    ...
                    for pattern_data in patterns:
                        email_to_test = pattern_data['email']
                        result = verifier.verify(email_to_test)
                        
                        # Prioritize SMTP-verified emails
                        if result.smtp_verified is True:
                            verified_email = email_to_test
                            best_result = result
                            break
                        
                        # Otherwise, track the best confidence
                        if not best_result or result.confidence_score > best_result.confidence_score:
                            best_email = email_to_test
                            best_result = result
                    ...
                    if delay > 0 and idx < total_rows - 1:
                        time.sleep(delay)
                
                # Combine results
                results_df = pd.DataFrame(results)
                final_df = pd.concat([df, results_df[['Found_Email', 'Status', 'Confidence', 'SMTP_Verified', 'Method']]], axis=1)
                
                # Show statistics
                verified_count = len([r for r in results if r.get('SMTP_Verified') == '✅'])
                total_found = len([r for r in results if r['Found_Email'] != 'NOT_FOUND'])
                
                st.success(f"✅ Complete! Found {verified_count}/{total_rows} verified emails ({total_found} total matches)")
                ...
                st.download_button(
                    "📥 Download Results", 
                    csv, 
                    "bulk_find_results.csv", 
                    "text/csv",
                    key="download_bulk_find"
                )
...
def show_bulk_find_tab(verifier, user, delay):
    """Bulk find emails from CSV with names and domains - STRICT VERSION"""
    st.markdown("### 📁 Bulk Find Emails from CSV")
    st.caption("Upload a CSV with FirstName, LastName, Domain columns - ONLY returns verified emails")
        ...
        try:
            ...
            if st.button("🔍 Find Emails in Bulk", type="primary"):
                ...
                for idx, row in df.iterrows():
                    ...
                    # ONLY look for SMTP-verified emails
                    for pattern_data in patterns:
                        email_to_test = pattern_data['email']
                        result = verifier.verify(email_to_test)
                        
                        # ONLY accept if SMTP verification is explicitly True
                        if result.smtp_verified is True:
                            verified_email = email_to_test
                    ...
                    if verified_email:
                        results.append({
                            ...
                            'LastName': last_name,
                            'Domain': domain,
                            'Found_Email': verified_email,
                            'Status': verified_result.status,
                            'SMTP_Verified': '✅',
                            'Confidence': f"{verified_result.confidence_score}%"
                        })
                    else:
                        results.append({
                            'FirstName': first_name,
                ...
                # Combine results
                results_df = pd.DataFrame(results)
                final_df = pd.concat([df, results_df[['Found_Email', 'Status', 'Confidence', 'SMTP_Verified']]], axis=1)
                ...
                if verified_count > 0:
                    st.success(f"✅ Found {verified_count}/{total_rows} verified emails!")
                else:
                    st.warning(f"❌ No verified emails found out of {total_rows} attempts")
        ...
        if st.button("Process", type="primary"):
            ...
            for i, email in enumerate(emails):
                ...
                results.append({
                    'Status': result.status,
                    'Deliverable': '✅' if result.deliverable else '❌',
                    'SMTP_Verified': '✅' if result.smtp_verified else '❌' if result.smtp_verified is False else '❓',
...
def main():
    ...
    # Tabs - THIS SHOULD BE IN THE MAIN FUNCTION
    ...
    with tab3:
        show_bulk_find_tab(verifier, user, delay)
